...
public:
  ...
  void exportToBMP(std::string path) {
    stbi_write_bmp(path.c_str(), getWidth(), getHeight(), 3, static_cast<void*>(getPixels()));
  }

  void exportToJPEG(std::string path, int quality = 90) {
    stbi_write_jpg(path.c_str(), getWidth(), getHeight(), 3, static_cast<void*>(getPixels()), quality);
  }
  ...
  void exportToPNG(std::string file) {
    stbi_write_png(file.c_str(), getWidth(), getHeight(), 3, static_cast<void*>(getPixels()), getWidth());
  }
